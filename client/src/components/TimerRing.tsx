  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      >
        {/* Background Ring */}
        <circle
          className="stroke-gh-border-muted"
          strokeWidth={stroke}
          fill="transparent"
          r={normalizedRadius}
          cx={radius}
        ...
        {/* Progress Ring */}
        <circle
          className="stroke-gh-danger transition-all duration-1000 ease-linear"
          strokeWidth={stroke}
      ...
      <div
        className={`absolute inset-0 rounded-full blur-[60px] opacity-20 pointer-events-none
          ${customColor ? customColor.replace('stroke-', 'bg-').replace('text-', 'bg-') : (mode === TimerMode.WORK ? 'bg-gh-danger-emph' : mode === TimerMode.SHORT_BREAK ? 'bg-gh-accent-emph' : 'bg-gh-success-emph')}
        `}
      />
    </div>
  );
