  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-gh-muted">Loading translations...</div>
      </div>
    );
  }
  ...
  return (
    <div className="language-switcher-container relative">
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded text-sm font-medium text-gh-fg hover:text-white hover:bg-gh-surface/50 transition-all border border-gh-border hover:border-gh-fg/30 w-[70px] justify-between"
      >
      ...
      {/* 下拉菜单 */}
      {isOpen && (
        <div className="absolute top-full right-0 mt-1 w-[100px] bg-gh-surface border border-gh-border rounded-lg shadow-lg z-50 overflow-hidden">
          {languageOptions.map((option) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 text-sm transition-all flex items-center justify-between ${language === option.code
                  ? 'bg-gh-accent-emph text-white'
                  : 'text-gh-fg hover:text-white hover:bg-gh-surface'
                }`}
            >
