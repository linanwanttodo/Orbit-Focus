// Vercel API - 路由处理
// 使用 @libsql/client 执行 SQLite 兼容查询

import { getDb, initializeDatabase } from './database';
import { jsonResponse, generateId, getNow, corsResponse, safeJsonParse, calculateStreak, getLast7DaysRange } from '../shared/utils';
...
export async function handleRequest(request: Request): Promise<Response> {
  try {
    ...
    // Health check
    if (path === '/api/health') {
      return jsonResponse({ status: 'ok', timestamp: getNow(), platform: 'vercel' });
    }
    ...
    if (path === '/api/tasks') {
      if (method === 'GET') {
        try {
          ...
          return jsonResponse(result.rows.map((r: any) => ({
            id: r.id,
            title: r.title,
      ...
      if (method === 'POST') {
        ...
        if (error) return error;          try {
            const id = generateId('task');
            const now = getNow();
            const title = String(body.title || '');
            const description = String(body.description || '');
            await db.execute({
              sql: 'INSERT INTO tasks (id, title, description, is_completed, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
              args: [id, title, description, 0, now, now],
            });
    ...
    if (taskMatch) {
      ...
      if (method === 'PUT') {
        ...
        try {            await db.execute({
            sql: 'UPDATE tasks SET title = ?, description = ?, is_completed = ?, updated_at = ? WHERE id = ?',
            args: [String(body.title || ''), String(body.description || ''), body.isCompleted ? 1 : 0, getNow(), id],
          });
          return jsonResponse({ success: true });
        } catch {
    ...
    if (path === '/api/sessions') {
      ...
      if (method === 'POST') {
        ...
        if (error) return error;          try {
            ...
            const now = getNow();
            await db.execute({
              sql: 'INSERT INTO sessions (id, type, duration, start_time, end_time, is_completed, work_time, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
              args: [id, String(body.type || 'work'), Number(body.duration || 0), String(body.startTime || now), body.endTime ? String(body.endTime) : null, 1, Number(body.workTime || body.duration || 0), now, now],
            });
    ...
    if (path === '/api/sessions/stats') {
      try {
        ...
        // Batch: all-time total + 7-day total + last 7 days + streak dates + heatmap (5 parallel queries)
        ...
        const [totalResult, weeklyTotalResult, dailyResult, streakResult, heatmapResult] = await Promise.all([
          ...
          db.execute({
            sql: 'SELECT SUM(work_time) as total FROM sessions WHERE is_completed = 1 AND date(start_time) >= ?',
            args: [sevenDaysAgo],
          }),
          db.execute({
            sql: 'SELECT date(start_time) as date, SUM(work_time) as work_time FROM sessions WHERE is_completed = 1 AND date(start_time) >= ? GROUP BY date(start_time) ORDER BY date ASC',
            args: [sevenDaysAgo],
          }),
          db.execute(
          ...
          db.execute({
            sql: 'SELECT date(start_time) as date, SUM(work_time) as work_time, COUNT(*) as sessions_count FROM sessions WHERE is_completed = 1 AND date(start_time) >= ? GROUP BY date(start_time) ORDER BY date ASC',
            args: [oneYearAgo.toISOString()],
          }),
        ]);

        const totalWorkTime = Number(totalResult.rows[0]?.total || 0);
        const weeklyWorkTime = Number(weeklyTotalResult.rows[0]?.total || 0);
        const streak = calculateStreak(streakResult.rows as unknown as { date: string }[]);
        const heatmapData = heatmapResult.rows.map((r: any) => ({
          date: r.date,
